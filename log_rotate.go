...
func SetLogRotateTask(d time.Duration) {
	...
	go func() {
		defer func() {
			if r := recover(); r != nil {
				ErrorLog.Println("[recover] log rotate task error", r)
			}
		}()
		logRotateTask(d)
	}()
...
func createNewOutput(redirectLv level, duration time.Duration) *os.File {
	...
	TraceLog.Println("open new log file:level", getLevelMsg(redirectLv), newOutput.Name())
...
func (l *iLogger) logRotate() {
	...
	default:
		f := oldOutput.(*os.File)
		TraceLog.Println("close log file:lv", getLevelMsg(l.levelFile), f.Name())
		f.Close()
	}
}
